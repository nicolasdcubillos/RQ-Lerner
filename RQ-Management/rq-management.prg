...
FOR lnColumn = 1 TO countColumnasSaldos
	...
	IF lnColumn % 2 = 1 && Valores para columnas de existencias
		lcNombreGrupo = lcFieldsTb2[lnColumn, 1]
		lcColor = RGB(206, 231, 230)
		lcText = " - Existencias"
		lcReadOnly = .T.
		lcBold = .T.
	ELSE				&& Valores para columnas de asignación
		lcNombreGrupo = SUBSTR(lcFieldsTb2[lnColumn, 1], 1, LEN(lcFieldsTb2[lnColumn, 1]) - 5)
		lcColor = RGB(206, 231, 255)
		lcText = " - Asignación"
		lcReadOnly = .F.
		lcBold = .F.
	ENDIF
